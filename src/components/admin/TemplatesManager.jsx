...
import { HiPlus, HiSearch, HiTemplate, HiPencil, HiTrash, HiOutlineColorSwatch } from 'react-icons/hi';
...
export default function TemplatesManager() {
  ...
  const handleEdit = (templateId) => {
    router.push(`/admin/template-builder/${templateId}`);
  };

  const handleNewTemplate = () => {
    router.push('/admin/template-builder/new');
  };
    ...
    <div className="glass rounded-2xl p-8 border border-white/5">
      <div className="flex flex-col md:flex-row items-center justify-between gap-4 mb-8">
        <div>
          ...
          <p className="text-sm text-white/50">Build professional Canva-style layered templates</p>
        </div>
        ...
        <div className="flex items-center gap-4 w-full md:w-auto">
          ...
          <button 
            ...
            className="flex items-center gap-2 px-4 py-2 bg-gradient-to-r from-indigo-600 to-purple-600 hover:from-indigo-500 hover:to-purple-500 text-white rounded-lg text-sm font-medium transition-all shadow-lg shadow-indigo-500/20 whitespace-nowrap"
          >
            <MdOutlineDesignServices size={18} /> Visual Builder
          </button>
        </div>
      </div>
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="border-b border-white/10 text-xs uppercase text-white/40">
                ...
                <th className="pb-3 font-medium">Elements</th>
            ...
            <tbody className="divide-y divide-white/5">
              {templates.map((t) => {
                const elementsCount = t.templateJson?.elements?.length ||
                  t.templateJson?.editableRegions?.length || 0;
                const hasNewFormat = !!t.templateJson?.elements;
                ...
                return (
                  <tr key={t.id} className="hover:bg-white/5 transition-colors group">
                    ...
                    <td className="py-4 text-sm text-white/60">
                      <span className="px-2 py-0.5 bg-white/10 rounded-full text-xs">{elementsCount}</span>
                      {hasNewFormat && (
                        <span className="ml-1.5 px-1.5 py-0.5 bg-indigo-500/20 text-indigo-400 rounded text-[10px] font-medium">v2</span>
                      )}
                    </td>
                    <td className="py-4">
                    ...
                    <td className="py-4 pr-2 text-right">
                      <div className="flex justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                        <button 
                          ...
                          className="flex items-center gap-1 px-2 py-1.5 text-indigo-300 hover:text-white bg-indigo-500/10 hover:bg-indigo-500/30 rounded text-xs font-medium transition-colors"
                        >
                          <MdOutlineDesignServices size={14} /> Edit
                        </button>
                        <button 
